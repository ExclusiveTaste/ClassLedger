...
当前尚未实现批量扣课、最终月度课时总表、记录筛选、学生详情、数据导入导出和清空数据。

下一阶段：**阶段四，具体范围以新的阶段任务为准**。
...
| 页面 | 当前状态 | 用途 |
| --- | --- | --- |
| 课时总表 | 基础框架完成 | 后续实现最终月度课时统计、筛选和批量扣课 |
...
- 当前自动化浏览器安全策略拒绝访问 `file://`，本轮真实浏览器 CRUD、视觉和响应式回归需要在本机双击 `index.html` 人工确认。
